...
def rearrange(puzzle):
    ...
    @details The rearrange function defines the blocks of three rows, sections, which comprise the puzzle.
    The function then defines the amount of zeros in each block. The sections are then rearranged in ascending order
    based on the number of zeros. They are concatenated into in a numpy array in this arrangement and the keys
...
def arrange_back(puzzle, keys):
    """
    @brief The rearrange function takes in the keys used to arrange the function as well as the solved puzzle and
    returns the solved puzzle with the row blocks rearranged to the original arrangement.
    @details The rearrange function takes in the list of keys which describe the rearrangement of the puzzle based on
    the rearrange function. The function then loops through to create a dictionary with the keys from the rearrange
    function and the row blocks of the solved puzzle. Sorting is used to rearrange the puzzle based on the key values.
    The puzzle is then concatenated as a numpy array in the original order of its row blocks.
